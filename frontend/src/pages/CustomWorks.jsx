...
export default function CustomWorks() {
    ...
    const submit = async () => {
        ...
        try {
            ...
            toast.success("Application sent!"); setApplying(null); setApply({ message: "", quoted_price_usd: 0 });
            fetch();
                    ...
                    <div key={w.id} className="bg-white border-2 border-[#1A1A1A] hard-shadow p-6" data-testid={`work-${w.id}`}>
                        <div className="flex flex-wrap gap-4 justify-between items-start">
                            ...
                            <div className="flex flex-col items-end gap-2">
                                <div className="font-heading font-black text-3xl inline-flex items-center"><IndianRupee className="w-6 h-6" />{w.budget_usd}</div>
                        ...
                        {applying === w.id && (
                            <div className="mt-4 pt-4 border-t-2 border-[#1A1A1A] space-y-3">
                                ...
                                <div className="flex gap-2">
                                    <input type="number" placeholder="Your quote $" value={apply.quoted_price_usd} onChange={(e) => setApply({ ...apply, quoted_price_usd: e.target.value })} className="px-3 py-2 border-2 border-[#1A1A1A] bg-[#F7F5F0] flex-1" data-testid="apply-quote" />
